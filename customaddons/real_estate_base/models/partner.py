...
#                partner.partner_attachments_url = path
    ...
    recomm = fields.Char('Recommandations', size=32)
    adresse = fields.Char('Adresse', size=32)
    situation = fields.Selection([('Celibataire sans enfant','Celibataire sans enfant'),
                                  ('Celibataire avec enfant','Celibataire avec enfant'),
                                  ('marié','marié')],
                               select=True)
    ...
    domicil = fields.Char('Banque de domiciliation', size=32)
    acquis  = fields.Char('Budget Acquisition', size=32)
    epargne = fields.Char('Montant epargne disponible', size=32)
    ageretraite = fields.Char('ageretraite', size=32)

    apport = fields.Boolean()
    coaquereur = fields.Boolean()
    achat_comptant = fields.Boolean()
    avaliste = fields.Boolean()
    co_emprunteur = fields.Boolean()
    autres = fields.Char('Autres revenus')
    quotite = fields.Char('Quotité Cessible Salaires(s)')
    resultat = fields.Boolean('Resultat Analyse Solvabilite Financiere')
    choix = fields.Selection([('Appartement', 'Appartement'),
                                         ('Maison', 'Maison')
                                         ], 'Type', required=True,
                                        select=True)
